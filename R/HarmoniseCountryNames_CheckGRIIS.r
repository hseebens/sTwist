...
griis_all <- griis_raw[,c("country","island_territory","scientificName", 
                          ...
                          # colnames(griis_raw)[grep("Introduced_before",colnames(griis_raw))],
...
glonaf$Country_GloNAF <- glonaf$Country  
  ...
  firstrecords$Country_FR <- firstrecords$Country
  ...
  colnames(firstrecords)[1] <- "Region"
  ...
  griis$Region <- griis$Country
  griis$Region[griis$island_territory!=""] <- griis$island_territory[griis$island_territory!=""]

  colnames(griis)[1] <- "Country_GRIIS"
  ...
  gavia$Country_GAVIA <- gavia$Country  
  ...
  amphrep_country$Country_AmpRep <- amphrep_country$Code  
  ...
  colnames(amphrep_country)[3] <- "Region"
  amphrep <- amphrep_country[,c("Country_AmpRep","DB_AmpRep","Region")]


  
  ### all database countries ##############
  
  alldat <- merge(griis,firstrecords,by="Region",all=T)
  alldat <- merge(alldat,glonaf,by="Region",all=T)
  alldat <- merge(alldat,gavia,by="Region",all=T)
  alldat <- merge(alldat,amphrep,by="Region",all=T)
  
  alldat[is.na(alldat)] <- ""

  alldat <- alldat[,c("Region","Country_GRIIS","Country_FR","Country_GloNAF","Country_GAVIA","Country_AmpRep")]
    
  colnames(alldat)[1] <- "Region_GRIIS"
  
  # write.table(alldat,"Data/RegionsAllDBs.csv",sep=",",row.names=F)
